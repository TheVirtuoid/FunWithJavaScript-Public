...
export default class App {
	...
	// #gravityVector = new Vector3(0, -39.24, 0);
...
*/
	...
	// inspiration from https://playground.babylonjs.com/#Z8HTUN#1093
	#testCreate() {
		const chassisLength = 10;
		...
		carParent.rotation.y = -Math.PI / 2;

		// Create a sphere shape and the associated body. Size will be de3.14termined automatically.
	...
	#insertMarble() {
		...
		marble.position = new Vector3(0, -7, 10);
		...
		new PhysicsAggregate(marble, PhysicsShapeType.SPHERE, { mass: 1, restitution: 0, friction: 0 }, this.#scene);
	}
	...
	/*
		...
		result.meshes[0].position = new Vector3(0, 6, 0);
		this.#testCar = result;
		// result.meshes[0].parent = this.#carParent;
		console.log(result.meshes[0]);
		new PhysicsAggregate(result.meshes[0], PhysicsShapeType.BOX, { mass: 1, restitution: .1, friction: 0}, this.#scene);
		...
		result.meshes[0].addChild(collisionMesh);
		return result;
	}
	 */
}
