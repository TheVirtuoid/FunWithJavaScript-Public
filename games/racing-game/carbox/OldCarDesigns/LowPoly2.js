...
export default class LowPoly2 {
	...
	static FRONT_LEFT_WHEEL_DATA = { xMultiplier: 1, zMultiplier: -1, name: 'front-left', key: LowPoly2.FRONT_LEFT_WHEEL };
	static FRONT_RIGHT_WHEEL_DATA = { xMultiplier: 1, zMultiplier: 1, name: 'front-right', key: LowPoly2.FRONT_RIGHT_WHEEL };
	static BACK_LEFT_WHEEL_DATA = { xMultiplier: -1, zMultiplier: -1, name: 'back-left', key: LowPoly2.BACK_LEFT_WHEEL };
	static BACK_RIGHT_WHEEL_DATA = { xMultiplier: -1, zMultiplier: 1, name: 'back-right', key: LowPoly2.BACK_RIGHT_WHEEL };
	...
	static WHEEL_DATA = new Map([
		[LowPoly2.BACK_LEFT_WHEEL, LowPoly2.BACK_LEFT_WHEEL_DATA],
		[LowPoly2.BACK_RIGHT_WHEEL, LowPoly2.BACK_RIGHT_WHEEL_DATA],
		[LowPoly2.FRONT_LEFT_WHEEL, LowPoly2.FRONT_LEFT_WHEEL_DATA],
		[LowPoly2.FRONT_RIGHT_WHEEL, LowPoly2.FRONT_RIGHT_WHEEL_DATA]
	]);
	...
	constructor(args = {}) {
		...
		this.#scene = scene;
		this.#scale = scale || LowPoly2.SCALE;
	...
	#lowPoly2WheelOffsets = new Map([
		[LowPoly2.BACK_LEFT_WHEEL, { x: -.1, y: 0, z: -.675 }],
		[LowPoly2.BACK_RIGHT_WHEEL, { x: -.1, y: 0, z: .675 }],
		[LowPoly2.FRONT_LEFT_WHEEL, { x: .1, y: 0, z: -.675 }],
		[LowPoly2.FRONT_RIGHT_WHEEL, { x: .1, y: 0, z: .675 }]
	]);
	...
	#buildWheelMaterial() {
		...
		const wheelRadius = (this.#modelDimensions?.wheelHeight || LowPoly2.WHEEL_RADIUS);
	...
	#buildWheel(wheelData) {
		...
		wheelHeight = wheelHeight || LowPoly2.WHEEL_HEIGHT;
		...
		const wheelOffset = this.#lowPoly2WheelOffsets.get(key);
	...
	#buildWheels() {
		this.#wheels = new Map();
		this.#wheels.set(LowPoly2.BACK_LEFT_WHEEL, this.#buildWheel(LowPoly2.BACK_LEFT_WHEEL_DATA));
		this.#wheels.set(LowPoly2.BACK_RIGHT_WHEEL, this.#buildWheel(LowPoly2.BACK_RIGHT_WHEEL_DATA));
		this.#wheels.set(LowPoly2.FRONT_LEFT_WHEEL, this.#buildWheel(LowPoly2.FRONT_LEFT_WHEEL_DATA));
		this.#wheels.set(LowPoly2.FRONT_RIGHT_WHEEL, this.#buildWheel(LowPoly2.FRONT_RIGHT_WHEEL_DATA));
	}
	...
	#buildChassis() {
		...
		this.#chassis.isVisible = !LowPoly2.HIDE_CHASSIS;
	}
	...
	#applyChassisPhysics() {
		...
		this.#aggregates.set(LowPoly2.CHASSIS, chassisAggregate);
	}
	...
	#applyWheelPhysics(wheel) {
		const aggregate = new PhysicsAggregate(
			...
			{
				mass: LowPoly2.WHEEL_MASS,
				restitution: LowPoly2.WHEEL_RESTITUTION,
				friction: LowPoly2.WHEEL_FRICTION
			},
			this.scene
		);
	...
	#setWheelConstraint(wheel, wheelPointer) {
		...
		const wheelData = LowPoly2.WHEEL_DATA.get(wheelPointer);
		...
		this.#aggregates.get(LowPoly2.CHASSIS)
			.body
	...
	async #buildModel() {
		this.#loadedModel = await ImportMeshAsync("/databases/car/LowPoly2.glb", this.#scene, {});
		...
		// 6 = car chassis floor
		// 7 = all four wheels?
		this.#loadedModel.meshes[7].isVisible = false;
		...
		this.#modelRoot.getChildMeshes().forEach((mesh, index) => {
			if (mesh.isVisible) mesh.isVisible = !LowPoly2.HIDE_MODEL;
		});
	...
	#buildCollisionBox() {
		...
		this.#collisionBox.isVisible = !LowPoly2.HIDE_COLLISION_BOX;
	}
	...
	#applyCollisionBoxPhysics() {
		...
		aggregate.shape.filterMembershipMask = this.#membershipMask;
		aggregate.shape.filterCollideMask = this.#collideMask;
		this.#aggregates.set(LowPoly2.MODEL, aggregate);
	}

	#setCollisionBoxConstraint() {
		const aggregate = this.#aggregates.get(LowPoly2.MODEL);
