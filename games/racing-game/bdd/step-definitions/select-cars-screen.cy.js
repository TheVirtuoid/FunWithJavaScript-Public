...
When('I select a car from the selected list', () => {
	cy.get('[data-testid=car-selected-list] li:first-child button').click();
});
...
Then('I should see the "Select Cars" title', (title) => {
	cy.get(`[data-testid=select-cars-title]`).should('be.visible');
});
...
Then('I should NOT see the car in the Selected cars list', () => {
	cy.get('[data-testid=car-selected-list] li').should('not.exist');
});
...
Then('I should NOT be able to click on any cars in the Selection list', () => {
	...
	cy.get('[data-testid=button-select-car]').should('be.disabled');
});
